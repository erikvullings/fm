      ...
      if (entries.length === 0) {
        context.setChecksumState(
          withChecksumError(
            ...
            'Select one or more files to calculate checksums for.',
          ),
        );
        context.redraw();
        return;
      ...
      void context
        ...
        .catch((error: unknown) => {
          context.setChecksumState(
            withChecksumError(
              context.getChecksumState(),
              message(error, 'Unable to start checksum calculation'),
            ),
          );
          context.redraw();
        });
      ...
      void context
        ...
        .catch((error: unknown) => {
          context.setChecksumState(
            withChecksumError(
              context.getChecksumState(),
              message(error, 'Unable to verify against that checksum file'),
            ),
          );
          context.redraw();
        });
        ...
        .catch((error: unknown) => {
          context.setDuplicateState(
            withDuplicateError(
              ...
              message(error, 'Unable to start duplicate detection'),
            ),
          );
          context.redraw();
        });
