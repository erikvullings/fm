...
export const DuplicateReviewView: FactoryComponent<DuplicateReviewViewAttrs> = () => ({
  view: ({ attrs }) => {
    ...
    return m('section.duplicate-review', { 'aria-label': 'Duplicate files' }, [
      m('header.duplicate-review__header', [
        m('h2', 'Duplicate files'),
        m(
          'span.duplicate-review__summary',
          attrs.isCancelled
            ? 'Scan cancelled — results are incomplete'
            : attrs.isComplete
              ? `${attrs.groups.length} groups, up to ${bytes(attrs.totalReclaimableBytes)} reclaimable`
              : 'Scanning…',
        ),
        !attrs.isComplete &&
          m(
      ...
      attrs.warningsCount > 0 &&
        m(
          ...
          `${attrs.warningsCount} file(s) could not be read and were skipped.`,
        ),

      attrs.isComplete &&
        attrs.groups.length === 0 &&
        !attrs.isCancelled &&
        m('p.duplicate-review__empty', 'No duplicate files were found.'),
        ...
        attrs.groups.map((group) =>
          m('li.duplicate-review__group', { key: group.fullHash }, [
            m('h3.duplicate-review__group-title', [
              `${bytes(group.size)} each`,
              m(
                'span.duplicate-review__reclaimable',
                ` · ${bytes(group.reclaimableBytes)} reclaimable`,
              ),
            ]),
                ...
                group.distinctLocations.map((location) => {
                  ...
                  return m('li', { key: location.uri }, [
                    m('label', [
                      ...
                      last &&
                        m('span.duplicate-review__keep-hint', ' (last remaining copy — kept)'),
                    ]),
                  ]);
                }),
              ),
            ...
            ...group.hardlinkClusters.map((cluster) =>
              m('div.duplicate-review__hardlinks', [
                m(
                  'p.duplicate-review__hardlink-note',
                  'Hardlinked — these paths are one file; deleting a path frees no space.',
                ),
                m(
                  'ul.duplicate-review__copies',
      ...
      m('footer.duplicate-review__footer', [
        m(
          ...
          selectedCount === 0
            ...
            : `Delete ${selectedCount} selected duplicate(s)…`,
        ),
      ]),
    ]);
  },
