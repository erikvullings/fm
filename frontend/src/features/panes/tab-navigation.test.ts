...
import { cycledTabIndex, reorderedTabIds, tabIdForJump } from './tab-navigation';
...
describe('reorderedTabIds', () => {
  it('moves the dragged tab immediately before the drop target', () => {
    expect(reorderedTabIds(['a', 'b', 'c', 'd'], 'd', 'b')).toEqual(['a', 'd', 'b', 'c']);
  });

  it('moves a tab forward past its target', () => {
    expect(reorderedTabIds(['a', 'b', 'c', 'd'], 'a', 'c')).toEqual(['b', 'a', 'c', 'd']);
  });

  it('is a no-op when dropping onto itself', () => {
    expect(reorderedTabIds(['a', 'b', 'c'], 'b', 'b')).toEqual(['a', 'b', 'c']);
  });

  it('returns the original order for an unknown target', () => {
    expect(reorderedTabIds(['a', 'b', 'c'], 'a', 'missing')).toEqual(['a', 'b', 'c']);
  });
});
