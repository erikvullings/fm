...
export function validatePasteTarget(
  ...
  if (target === undefined || !target.loaded) {
    return { ok: false, message: 'The destination directory is unavailable.' };
  }
  if (!target.writable) {
    return { ok: false, message: 'The destination directory is read-only.' };
  }
  if (clipboard.locations.some((source) => isSameOrDescendant(source, target.location))) {
    return { ok: false, message: 'Cannot paste a location into itself or its subtree.' };
  }
  return { ok: true };
}
