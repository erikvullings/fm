...
export const SessionTokenGate: FactoryComponent<SessionTokenGateAttrs> = () => {
  ...
  setUnauthorizedHandler(() => {
    ...
    state.error = 'The server rejected that token. It may have restarted; paste the current one.';
    m.redraw();
  });
    ...
    view: ({ attrs }) => {
      ...
      return m('.fm-session-token-gate', [
        m('form.fm-session-token-gate__card', { onsubmit: submit }, [
          m('h1', 'Sign in to fm-server'),
          m(
            'p',
            'This server requires a session token. Paste the token fm-server printed to its ' +
              'console at startup — it is not stored anywhere except this browser tab.',
          ),
