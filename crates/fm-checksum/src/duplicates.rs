...
//! [`fm_vfs::ProviderRegistry`]), but identity is not: `(device, inode)` is a
//! Unix concept obtained through [`std::os::unix::fs::MetadataExt`]. On
//! non-Unix platforms, and for any non-local provider, identity is reported
//! as unknown and every file is treated as a distinct file — the detector
//! never *claims* a hardlink it cannot prove.
...
impl FileIdentity {
    ...
    #[cfg(unix)]
    #[must_use]
    fn of_path(path: &std::path::Path) -> Option<Self> {
    ...
    /// Non-Unix platforms expose no portable inode number through `std`, so
    /// identity is reported as unknown rather than guessed. Every file is
    /// then treated as distinct, which is the safe direction: the detector
    /// under-reports hardlinks instead of falsely claiming them.
    #[cfg(not(unix))]
    #[must_use]
    fn of_path(_path: &std::path::Path) -> Option<Self> {
        None
    }
}
