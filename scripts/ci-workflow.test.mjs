...
test("rust job runs fmt --check, clippy -D warnings and cargo test --workspace", () => {
  const commands = allRunCommands(workflow.jobs.rust);
  assert.ok(commands.some((c) => /cargo fmt --all --check/.test(c)));
  assert.ok(
    commands.some((c) =>
      /cargo clippy --workspace --all-targets -- -D warnings/.test(c),
    ),
  );
  assert.ok(commands.some((c) => /cargo test --workspace/.test(c)));
});

test("rust job caches the cargo registry and target directory", () => {
  const uses = (workflow.jobs.rust.steps ?? [])
    .map((step) => step.uses)
    .filter(Boolean);
  assert.ok(
