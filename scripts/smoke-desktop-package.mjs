...
async function smokeMacos() {
  ...
  try {
    ...
    await assertLaunches(join(installedApp, 'Contents', 'MacOS', 'fm-desktop'));
  } finally {
...
async function smokeWindows() {
  ...
  try {
    ...
    const executable = filesBelow(installRoot).find(
      (path) => basename(path).toLowerCase() === 'fm-desktop.exe',
    );
    if (!executable) throw new Error('MSI did not install fm-desktop.exe');
