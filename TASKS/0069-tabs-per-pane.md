...
- 2026-08-02 copilot: Re-verified a substantial, high-quality implementation left uncommitted by a
  ...
  - **Tab strip / drag reorder / close confirmation**: tab strip UI lives in
    `frontend/src/features/panes/pane.ts` (keyed vnodes per tab, `title` attr for the full-path
    tooltip, drag handlers wired to `reorderedTabIds` from the new
    `frontend/src/features/panes/tab-navigation.ts`). Drag-reorder is **frontend-only** — there is
    no backend `reorderTabs` workspace command, so `workspace-layout.ts`'s `tabOrderOverrides` map
    is discarded whenever the pane's real `tabOrder` changes (tab added/closed/reloaded). This is a
    deliberate, documented scope decision (matches the acceptance criteria's silence on reorder
    persistence), not a bug. The tab strip scrolls (`overflow-x: auto`) but has no dedicated fade/
    arrow overflow affordance beyond that — accepted as a reasonable minimal reading of "shows an
    overflow affordance". Closing a pane's only tab is gated behind a new
