...
| Ctrl+L | Calculate occupied disk space | Focus Location bar (`core.focusLocation`) | Keep fm's Ctrl+L. Disk-usage (task [0118](0118-integrate-parallel-disk-usage-windirstat.md), still open) should get its own shortcut when built — suggest Ctrl+Shift+L — rather than reclaiming Ctrl+L. |
