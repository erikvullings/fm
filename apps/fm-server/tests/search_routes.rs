...
/// Polls `/api/v1/directories/list` for `location` until the search reports
/// `hasMore: false`, returning the final listing body.
async fn poll_until_complete(client: &reqwest::Client, base_url: &str, location: &Value) -> Value {
    for _ in 0..100 {
        let response = client
        ...
        if body["hasMore"] == json!(false) {
            return body;
        }
        tokio::time::sleep(Duration::from_millis(20)).await;
    }
    panic!("search did not complete in time");
}
